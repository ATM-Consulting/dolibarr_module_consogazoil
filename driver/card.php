...
/**
 * \file consogazoil/driver/card.php
 * \ingroup consogazoil
...
if ($action == "create_confirm") {
	...
	if (empty($error)) {
		...
		if ($result < 0) {
			...
			header('Location:' . dol_buildpath('/consogazoil/driver/card.php', 1) . '?id=' . $result);
		}
	}
	...
	if ($result < 0) {
		...
		header('Location:' . dol_buildpath('/consogazoil/driver/list.php', 1));
	}
	...
	if ($result < 0) {
		...
		header('Location:' . dol_buildpath('/consogazoil/driver/card.php', 1) . '?id=' . $object->id);
	}
}
 ...
 * View
 */
$title = $langs->trans('Module103040Name') . '-' . $langs->trans('ConsoGazManageDriv');
if (! empty($object->ref))
	$title .= '-' . $object->ref;
